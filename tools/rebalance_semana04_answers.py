"""Permuta alternativas da Semana 4 sem alterar o conteúdo semântico.
...
SUPER_PATH = WEEK / "semana_04_super_simulado.md"
...
def answer_match(section: str) -> re.Match[str]:
    match = re.search(
        r"(?m)^\*\*(?:Alternativa correta|Gabarito):\*\*\s*([A-D])(?:\.?[^\r\n]*)?$"
        r"|^\*\*(?:Alternativa correta|Gabarito):\s*([A-D])(?:\b|\.)(?:[^*]*)\*\*\s*$",
        section,
    )
    if not match:
...
def load_items(paths: list[Path], super_mode: bool = False) -> tuple[list[Item], dict[Path, str]]:
    ...
    order = 0
    if super_mode:
        item_pattern = r"(S4S\d{3})\s+(?:—|-)\s+.+"
    else:
        item_pattern = r"(S4D[1-6]Q\d{3})\s+(?:—|-)\s+.+|((?:Extra Dia [1-6]\.\d+))(?:\s+(?:—|-)\s+.+)?"
    for path, source in sources.items():
        spans = section_spans(source, item_pattern) if super_mode else {}
        if super_mode:
            comment_spans = section_spans(source, r"Comentário (S4S\d{3})")
        else:
            comment_spans = {
                **section_spans(source, r"Comentário (S4D[1-6]Q\d{3})"),
        ...
        if not super_mode:
            ...
            rx = re.compile(r"(?m)^(#{1,6}) ((S4D[1-6]Q\d{3})|((?:Extra Dia [1-6]\.\d+)))(?:\s+(?:—|-)\s+.+)?\s*$")
            headings = list(re.finditer(r"(?m)^(#{1,6}) .+$", source))
...
def target_answers(items: list[Item], base_seed: int) -> dict[str, str]:
    ...
    for item in ordered:
        match = re.match(r"S4D([1-6])Q|Extra Dia ([1-6])\.", item.item_id)
        if match:
...
def replace_section(
    ...
    *,
    ...
    answer_group = next(
        index for index, value in enumerate(current_match.groups(), start=1) if value
    )
    answer_letter_start = current_match.start(answer_group)
...
def update_tables(source: str, targets: dict[str, str]) -> str:
    ...
    for index, line in enumerate(lines):
        ...
        # Linha vertical: | S4... | A |
        vertical = re.fullmatch(r"\|\s*(S4(?:D[1-6]Q|S)\d{3})\s*\|\s*([A-D])\s*\|", line.strip())
        ...
        if vertical_local and current_day is not None:
            ...
            item_id = f"S4D{current_day}Q{((current_day - 1) * 50 + local):03d}"
            if item_id in targets:
                lines[index] = f"| {local} | {targets[item_id]} |"
            continue

        ids = re.findall(r"S4(?:D[1-6]Q|S)\d{3}", line)
        if len(ids) >= 2:
            if index + 2 >= len(lines):
                raise ValueError("tabela horizontal truncada")
            # Faixas como ``S4S001–S4S010`` também contêm dois IDs, mas
...
def apply_targets(paths: list[Path], targets: dict[str, str], super_mode: bool = False) -> None:
    for path in paths:
        ...
        if super_mode:
            item_spans = section_spans(source, r"(S4S\d{3})\s+(?:—|-)\s+.+")
            comment_spans = section_spans(source, r"Comentário (S4S\d{3})")
        else:
            item_spans = {}
            item_rx = re.compile(r"(?m)^(#{1,6}) ((S4D[1-6]Q\d{3})|(Extra Dia [1-6]\.\d+))(?:\s+(?:—|-)\s+.+)?\s*$")
            headings = list(re.finditer(r"(?m)^(#{1,6}) .+$", source))
...
def main() -> None:
    ...
    bank_targets = target_answers(bank_items, base_seed=40_820_126)
    bank_changes = [
        (item.item_id, item.answer, bank_targets[item.item_id])
        for item in bank_items
        if item.answer != bank_targets[item.item_id]
    ]
    ...
    super_targets = target_answers(super_items, base_seed=64_820_126)
    super_changes = [
        (item.item_id, item.answer, super_targets[item.item_id])
        for item in super_items
        if item.answer != super_targets[item.item_id]
    ]
    ...
    print(f"Banco: {len(bank_changes)} posições alteradas.")
    for item_id, before, after in bank_changes:
        print(f"- {item_id}: {before} -> {after}")
    print(f"Super: {len(super_changes)} posições alteradas.")
    for item_id, before, after in super_changes:
        print(f"- {item_id}: {before} -> {after}")
