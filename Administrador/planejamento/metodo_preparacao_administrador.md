...
```mermaid
flowchart LR
    A[Edital e fonte vigente] --> B[Plano por peso]
    B --> C[Teoria estruturada]
    C --> D[Exemplo e ferramenta aplicada]
    D --> E[Até 10 questões essenciais]
    E --> F[Correção integral]
    F --> G[Caderno de erros]
    G --> H[Revisões D+2 e D+7]
    H --> I[Simulado e dissertação]
    I --> C
```
